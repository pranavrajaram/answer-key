            ...
            <div className="ak-card p-4 sm:p-6">
              <div className="mb-5 flex flex-col items-start gap-3 sm:flex-row sm:justify-between sm:gap-4">
                ...
                {isResolved ? (
                  ...
                  <span className="ak-badge bg-amber-50 text-amber-700">
                    Awaiting resolution
                  </span>
                )}
              </div>
                ...
                <div className="space-y-3">
                  {market.options.map((opt: string) => {
                    ...
                    return (
                      <div
                        key={opt}
                        className={`rounded-xl p-3 ${isWinner ? 'border border-teal-200 bg-teal-50' : 'bg-stone-50/80'}`}
                      >
                        <div className="flex justify-between items-center mb-2">
                          <span className={`text-sm font-medium ${isWinner ? 'text-teal-800' : 'text-stone-600'}`}>
                            {opt} {isWinner && '✓'}
                          </span>
                          <span className="text-xs text-stone-500">{optTotal} pts</span>
                        </div>
              ...
              <div className="ak-card p-4 sm:p-6">
                ...
                <div className="space-y-2">
                  {(bets ?? []).slice(0, 20).map((b: Bet) => (
                    <div key={b.id} className="flex items-center justify-between border-b border-stone-100/80 py-1.5 text-sm last:border-0">
                      <div className="flex items-center gap-2">
                        <span className={`text-xs font-medium ${b.user_id === user.id ? 'text-teal-700' : 'text-stone-600'}`}>
                          {b.profiles?.username ?? 'unknown'}
                          {b.user_id === user.id && <span className="ml-1 text-stone-400">you</span>}
                        </span>
                        <span className="text-stone-400">bet on</span>
                        <span className="font-medium text-stone-700">{b.option}</span>
                      </div>
                      <span className="tabular-nums text-stone-500">{b.amount} pts</span>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-1.5">
                  {positions.filter((p: { option: string; net: number }) => p.net > 0 || myBets.some((b: Bet) => b.option === p.option)).map((p: { option: string; net: number }) => (
                    <div key={p.option} className="flex justify-between text-sm">
                      <span className="text-stone-700">{p.option}</span>
                      <span className="font-medium text-stone-900 tabular-nums">
                        {p.net > 0 ? `${p.net} shares` : <span className="text-stone-400">sold out</span>}
                      </span>
                    </div>
                  ))}
                  <div className="flex justify-between border-t border-stone-200/70 pt-1.5 text-xs text-stone-500">
